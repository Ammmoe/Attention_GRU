...
Sequence-to-sequence LSTM model for multi-agent trajectory prediction in 2D or 3D space.
...
class TrajPredictor(nn.Module):
    """
    Sequence-to-sequence LSTM model for multi-agent trajectory prediction.

    Architecture:
        - Encoder LSTM: Encodes past trajectory for each agent.
        - Decoder LSTM: Autoregressively generates future trajectory.
        - Fully connected layer: Maps LSTM hidden states to output coordinates.
    ...
    def forward(self, src, tgt=None, pred_len=1):
        ...
        for agent_idx in range(num_agents):
            # ---- Encoder ----
            _, (h, c) = self.encoder(src_agents[agent_idx])
